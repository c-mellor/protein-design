...
def get_protein_sequence(struct_json_path: Path) -> Tuple[Dict[str, List[str]], str]:
    ...
    # Check the number of lines (i.e. json dicts) is 1.
    with struct_json_path.open("r") as f:
        for ii, _ in enumerate(f):
            if ii > 0:
                raise IndexError(
                    f'"{struct_json_path}" is formatted incorrectly, it contains multiple proteins. '
                    "This may occur when giving an input directory with multiple PDB files."
                    "You should use `split_mpnn_structure_json.py` to split the single JSON file into multiple different files."
                )
